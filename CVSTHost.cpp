...
bool CEffect::Load(LPCTSTR name)
{
...
if (pEffect)
  {
  sName = new TCHAR[lstrlen(name) + 1];
  if (sName)
    lstrcpy(sName, name);
#ifdef WIN32
  LPCTSTR p = _tcsrchr(name, '\\');
  if (p)
    {
...
pEffect->DECLARE_VST_DEPRECATED(process)(pEffect, inputs, outputs, sampleframes);
}
...
switch (opcode)
  ...
  case DECLARE_VST_DEPRECATED(audioMasterOpenWindow):
	  return (long)OnOpenWindow(nEffect, (DECLARE_VST_DEPRECATED(VstWindow) *)ptr);
  case DECLARE_VST_DEPRECATED(audioMasterCloseWindow):
	  return OnCloseWindow(nEffect, (DECLARE_VST_DEPRECATED(VstWindow) *)ptr);
...
int CVSTHost::LoadPlugin(LPCTSTR sName, int nUniqueId)
{
...
void * CVSTHost::OnOpenWindow(int nEffect, DECLARE_VST_DEPRECATED(VstWindow)* window)
{
CEffect *pEffect = GetAt(nEffect);
if (pEffect)
...
bool CVSTHost::OnCloseWindow(int nEffect, DECLARE_VST_DEPRECATED(VstWindow)* window)
{
CEffect *pEffect = GetAt(nEffect);
if (pEffect)
